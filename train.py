...
parser.add_argument("--model_type", type=str, default="mpnet")
...
parser.add_argument("--one_encoder", type=bool, default=True)
...
#optimizer = torch.optim.SGD(list(sentence_encoder.parameters()) + list(query_encoder.parameters()) + list(linear_query.parameters())
#+list(linear_sentence.parameters()), lr=1e-3, momentum=0.9)

#optimizer = torch.optim.SGD(list(sentence_encoder.parameters()) + list(query_encoder.parameters()), lr=0.001, momentum=0.9)

bitfit = False
if args.linear_is_unit:
    linear_query.weight.data = torch.eye(768)
    linear_sentence.weight.data = torch.eye(768)
    linear_query.bias.data = torch.zeros(768)
    linear_sentence.bias.data = torch.zeros(768)
...
update_embeddings_in_bitfit = True
...
if update_only_last_layers:
    ...
    for param in query_encoder.encoder.layer[-1].parameters():
        param.requires_grad = True
if bitfit: # update only bias terms
...
linear_sentence.to(device)
    # sentence_encoder.load_state_dict(torch.load("sentence_encoder3_{}_bitfit:{}_mean_v2.pt".format(model_type, False)))
    # query_encoder.load_state_dict(torch.load("query_encoder3_{}_bitfit:{}_mean_v2.pt".format(model_type,False)))
    # linear_query.load_state_dict(torch.load("linear_query3_{}_bitfit:{}_mean_v2.pt".format(model_type,False)))
    # linear_sentence.load_state_dict(torch.load("linear_sentence3_{}_bitfit:{}_mean_v2.pt".format(model_type,False)))
...
if args.load_existing:
    ...
    sentence_encoder_dict = torch.load("sentence_encoder3_{}_bitfit:{}_final_mean_v2.pt".format(args.model_type, False))
    ...
    query_encoder_dict = torch.load("query_encoder3_{}_bitfit:{}_final_mean_v2.pt".format(args.model_type,False))
    ...
    linear_query_dict = torch.load("linear_query3_{}_bitfit:{}_final_mean_no-negatives_v2.pt".format(args.model_type,False))
    ...
    linear_sentence_dict = torch.load("linear_sentence3_{}_bitfit:{}_final_mean_no-negatives_v2.pt".format(args.model_type,False))
...
for epoch in range(args.epochs):
    for i in range(0, len(train), args.batch_size):
        ...
        if freeze_all: # no need to take gradients
            with torch.no_grad():
                sentence_features = encode_batch(sentence_encoder, tokenizer, anchors, device, pooling="cls")
                positive_features = encode_batch(query_encoder, tokenizer, positives, device, pooling=args.pooling)
                negative_features = encode_batch(query_encoder, tokenizer, gold_negatives, device, pooling="cls")
        else:
            positive_features = encode_batch(query_encoder, tokenizer, positives, device, pooling="cls")
        ...
        #sentence_features = linear_sentence(sentence_features)
        #positive_features = linear_query(positive_features)
        #negative_features = linear_query(negative_features)
        ...
        #indices_tuple = (torch.arange(batch_size), torch.arange(batch_size, 2*batch_size), torch.arange(2*bathc_size, 3*batch_size))
        # if random.random() < 0.5:
        #     loss = loss_func(embeddings, batch_labels)
        #     #loss = info_nce_loss(sentence_features, positive_features)
        #     mean_loss_batch_hard.append(loss.detach().cpu().numpy().item())    
        # else:  
        #     loss = torch.nn.functional.relu(dist(sentence_features, positive_features) - dist(sentence_features, negative_features) + 0.1)
        #     loss = loss.mean()
        #     mean_loss_gpt_hard.append(loss.detach().cpu().numpy().item())
        ...
        if i % (args.batch_size * 10) == 0 and i > 0:
            # print("epoch: {}, batch: {}, loss: {}; loss-batch-hard: {}; loss-gpt-hard: {}".format(epoch, i, np.mean(mean_loss),
            #                                                                                      np.mean(mean_loss_batch_hard),
            #                                                                                      np.mean(mean_loss_gpt_hard)))
            ...
            if (mean_loss_val < best_loss) and (mean_loss_gpt_hard_val is not None) and (mean_loss_batch_hard_val is not None):
                ...
                print("saving model")
                torch.save(sentence_encoder.cpu().state_dict(), "sentence_encoder3_mpnet_bitfit:False_final_mean+cls_negatives:0.1_late:True_v2.pt")
                torch.save(query_encoder.cpu().state_dict(), "query_encoder3_mpnet_bitfit:False_final_mean+cls_negatives:0.1_late:True_v2.pt")
                torch.save(linear_sentence.cpu().state_dict(), "linear_query3_mpnet_bitfit:False_final_mean+cls_negatives:0.1_late:True_v2.pt")
                torch.save(linear_query.cpu().state_dict(), "linear_sentence3_mpnet_bitfit:False_final_mean+cls_negatives:0.1_late:True_v2.pt")
...
torch.save(sentence_encoder.cpu().state_dict(), "sentence_encoder5_{}_bitfit:{}_final_{}_negatives:{}_late:{}_v2.pt".format(args.model_type, bitfit, args.pooling, args.hard_negatives_weight, args.late_interaction))
torch.save(query_encoder.cpu().state_dict(), "query_encoder5_{}_bitfit:{}_final_{}_negatives:{}_late:{}_v2.pt".format(args.model_type, bitfit, args.pooling, args.hard_negatives_weight, args.late_interaction))
torch.save(linear_sentence.cpu().state_dict(), "linear_sentence5_{}_bitfit:{}_final_{}_negatives:{}_late:{}_v2.pt".format(args.model_type, bitfit, args.pooling, args.hard_negatives_weight, args.late_interaction))
torch.save(linear_query.cpu().state_dict(), "linear_query5_{}_bitfit:{}_final_{}_negatives:{}_late:{}_v2.pt".format(args.model_type, bitfit, args.pooling, args.hard_negatives_weight, args.late_interaction))